...
// let nameStundents = students.name;
// newArray = [];
// students.forEach((element, index, array) => {
   
//     newArray.push(nameStundents);
//     });
...
const targhe = students.map((element) =>{
    // prima soluzione con  PRIMO RETURN
    
    // return element.name.toUpperCase();
// IMPORTANTE CREANDO UN NUOVO ARRAY DI OGGETTI!!!!
...
const migliori = students.filter((element) =>{
    return element.grades > 70;
});
console.log(migliori);
// 3-  Dobbiamo creare un nuovo array di tutti gli studenti che hanno un totale di voti superiore a 70 e id superiore a 120

const miglioriId = students.filter((element) =>{
    return element.grades > 70 && element.id > 120;
});

console.log(miglioriId);